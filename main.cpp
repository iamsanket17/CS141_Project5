...
//  Author: Dale Reed
//  Editor/ Implementor: Sanket Patel
...
const sf::Color Black          = sf::Color( 0, 0, 0); // Dark Gray
...
void swapColors( Square & source, Square & destination )
{
...
void getAndValidateUserInput( 
                        ...
                        char &moveAction)       // Used to return a move action to retry, reset, or exit. 
{
        ...
        while( true) {
                ...
                if( direction == 'l') {
                        destinationIndex = squareIndex - 1; 
                }
                else if( direction == 'r' ) {
                        destinationIndex = squareIndex + 1;
                }
...
//---------------------------------------------------------------------------------------
// *** Yuwei: add header comments for this function to explain its purpose.
void markMatchingPieces(Square squaresArray[],Square pieceToMatch,int currentPosition, bool &matchesFound)
{
    // Check each neighbor, and set it to blackcolor if it matches. If it matches and is set to blackcolor,
    // then recursively repeat the process, from the perspective of that neighbor.
    // First check the left neighbor:
    if( squaresArray[currentPosition-1].getColor() == pieceToMatch.getColor() &&  squaresArray[currentPosition-1].getColor() != WallColor && squaresArray[currentPosition -1].getIsCaptured()!= true) {
        squaresArray[currentPosition-1].setIsCaptured(true);
        matchesFound = true;
        markMatchingPieces( squaresArray, pieceToMatch, currentPosition-1, matchesFound);
    ...
    if( squaresArray[currentPosition-10].getColor() == pieceToMatch.getColor() &&  squaresArray[currentPosition-10].getColor() != WallColor && squaresArray[currentPosition -10].getIsCaptured()!= true) {
        squaresArray[currentPosition-10].setIsCaptured(true);
        matchesFound = true;
        markMatchingPieces(squaresArray, pieceToMatch, currentPosition-10, matchesFound);
    }
...
}//end of markMatchingPieces


//---------------------------------------------------------------------------------------
//Blank out matching pieces previously set to blackcolor.
void blankOutMatchingPieces(Square squaresArray[], bool &matchesFound)
{
    for(int currentPosition=12; currentPosition<70; currentPosition++)
    {
        if(squaresArray[currentPosition].getIsCaptured()==true && matchesFound== true )
        {
           squaresArray[currentPosition].setColor(BlankRGBValue, BlankRGBValue,BlankRGBValue); 
          squaresArray[currentPosition].setIsCaptured(false);  
        }
        if(squaresArray[currentPosition].getIsCaptured()== true && matchesFound!= true ){
            squaresArray[currentPosition].setIsCaptured(false); 
        }
...
}//end of blankOutMatchingPieces


//---------------------------------------------------------------------------------------
// Find and delete adjacent matching pieces throughout the entire board.
// Do this in two steps:
//   1. Recursively mark matching pieces by setting them to blackcolor.
//   2. Clear all pieces previously set to blackcolor.


void findAndDeleteMatchingPieces(Square squaresArray[])
{
   // Look at each board piece, comparing it to its adjacent neighbors.
   for(int currentPosition=12; currentPosition<70; currentPosition++)
   {
     bool  matchesFound = false;
     if(squaresArray[currentPosition].getColor()!=WallColor && squaresArray[currentPosition].getColor()!=BlankColor)  
     {
         // Recursively mark any neighbor that matches as blackcolor.  Set matchesFound to true if any are found.
         squaresArray[currentPosition].setIsCaptured(true);
         markMatchingPieces(squaresArray ,squaresArray[currentPosition] ,currentPosition ,matchesFound);
     }
       // Go through board and blank out pieces set to blackcolor.
       blankOutMatchingPieces(squaresArray, matchesFound);
   }
}//end of findAndDeleteMatchingPieces.


//---------------------------------------------------------------------------------------
//Returns true when board is cleared of all pieces; false otherwise
int boardIsClear(Square squaresArray[])
{
    for(int currentPosition=12; currentPosition<70; currentPosition++)
    {
        if(squaresArray[currentPosition].getColor()!=BlankColor && squaresArray[currentPosition].getColor()!=WallColor)
        {
            return 1; //(false)
        }
    }
...
{        
        ...
        while (window.isOpen())
        {
                ...
                getAndValidateUserInput( squareIndex, destinationIndex, squaresArray, moveNumber, moveAction);
        ...
        // A valid move was chosen. Move the piece, by swapping the source and destination squares' colors.
        swapColors(squaresArray[squareIndex], squaresArray[destinationIndex]); 
          
        squareIndex=destinationIndex;    
        destinationIndex=squareIndex+10;
        
        //Code segment to check and move if the below index under the user-selected index is empty or not.
        while(squaresArray[destinationIndex].getColor()==BlankColor)
        {
            swapColors(squaresArray[squareIndex], squaresArray[destinationIndex]);
        ...
        //Loop for calling gravity and destroy-function repeatedly. 
        for(int loopindex=0;loopindex<118;loopindex++){
            //Code Segment (Gravity)
            for(int k=80;k>=0;k--)
            {
                if(squaresArray[k].getColor()==BlankColor)
                {
                    if(squaresArray[k-10].getColor()!=BlankColor && squaresArray[k-10].getColor()!=WallColor)
                    {
                         swapColors(squaresArray[k], squaresArray[k-10]);
                    }
                }
            }
           //Destroy function call.
           findAndDeleteMatchingPieces(squaresArray); 
         }
        // Check segment to check the board is clear or not.   
        if(boardIsClear(squaresArray)==2)
        {
// *** Yuwei: Common part of these two else-if branches can be extracted out.            
         if(moveNumber<=allTheBoards.getParValue(currentBoardIndex))
         { 
             score+= allTheBoards.getParValue(currentBoardIndex);               //Score increment if level is completed(board completed with in par values).
             std::cout<<"Congratulations!  On to the next level."<<std::endl;   //Display Congratulations message.
             std::cout<<"Score: "<<score<<std::endl;                            //Display score for the user.
             allTheBoards.setBoardValues( ++currentBoardIndex, boardValues);
             moveNumber = 0;
             // Reinitialize the displayWindow, using new board values to reset squaresArray values
             initializeDisplayWindow( window, backgroundRectangle, boardValues, squaresArray, messagesLabel, font, "Welcome");
         ...
         {
           std::cout<<"Sorry, you took 6 moves and you must finish within 4 moves before moving on."<<std::endl; 
           std::cout<<std::endl; 
           allTheBoards.setBoardValues( currentBoardIndex, boardValues);
           moveNumber = 0;
           // Reinitialize the displayWindow, using new board values to reset squaresArray values
